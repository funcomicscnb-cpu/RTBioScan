...
file_name <- args[1]
sample_name <- gsub("_consensus_consolidated_tax_gns_metazoa_treemap_clusters_rpt.txt", "", basename(file_name))
df <- read_treemap_df(file_name, c("Class", "Genus", "Clusters"))
output_file <- paste0(sample_name, "_consensus_consolidated_tax_gns_COI_treemap_clusters.png")
write_treemap(df, output_file, "Genus")

file_name <- args[2]
sample_name <- gsub("_consensus_consolidated_tax_gns_viridiplantae_treemap_clusters_rpt.txt", "", basename(file_name))
df <- read_treemap_df(file_name, c("Class", "Genus", "Clusters"))
output_file <- paste0(sample_name, "_consensus_consolidated_tax_gns_ITS_treemap_clusters.png")
write_treemap(df, output_file, "Genus")
