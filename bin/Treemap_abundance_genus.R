# Treemap plot
# Genus abundance in terms of reads
library(ggplot2)
...
if (length(args) == 2 && grepl("_treemap_rpt\\.txt$", args[2])) {
  file_name <- args[1]
  sample_name <- gsub("_otu_tax_gns_metazoa_treemap_rpt.txt","",basename(file_name))
  process_pair(file_name, paste0(sample_name, "_otu_tax_gns_COI_treemap.png"))

  file_name <- args[2]
  sample_name <- gsub("_otu_tax_gns_viridiplantae_treemap_rpt.txt","",basename(file_name))
  process_pair(file_name, paste0(sample_name, "_otu_tax_gns_ITS_treemap.png"))
} else if (length(args) >= 2 && length(args) %% 2 == 0) {
  for (i in seq(1, length(args), by = 2)) {
    process_pair(args[i], args[i + 1])
  }
} else {
  stop("Usage: Treemap_abundance_genus.R <legacy_file1> <legacy_file2> OR repeated <input> <output_png> pairs")
}
