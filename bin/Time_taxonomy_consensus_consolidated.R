...
file_name <- args[1]
#sample_name <- args[2]
sample_name <- gsub("_consensus_consolidated_tax_time_rpt.txt","",basename(file_name))

#data_cumulative <- read.delim("Tucan_3/Tucan_3_consensus_tax_time_rpt.txt")
...
if (nrow(parsed) == 0) {
  output_file <- paste0(sample_name, "_consensus_consolidated_tax_time.png")
  save_plot_placeholder(output_file, "Taxons identified over time in Consolidated Consensus")
  quit(status = 0)
}
...
p <- ggplot(parsed, aes(x = hours, y = y_plot, color = rank, linetype = series)) +
  ...
  labs(
    title = "Taxons identified over time in Consolidated Consensus",
    x = "Time (h)",
    color = NULL,
    linetype = NULL
  ) +
