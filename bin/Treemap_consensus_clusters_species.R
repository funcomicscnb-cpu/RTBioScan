...
file_name <- args[1]
sample_name <- gsub("_consensus_tax_spc_metazoa_treemap_clusters_rpt.txt", "", basename(file_name))
df <- read_treemap_df(file_name, c("Class", "Species", "Clusters"))
output_file <- paste0(sample_name, "_consensus_tax_spc_COI_treemap_clusters.png")
write_treemap(df, output_file, "Species")

file_name <- args[2]
sample_name <- gsub("_consensus_tax_spc_viridiplantae_treemap_clusters_rpt.txt", "", basename(file_name))
df <- read_treemap_df(file_name, c("Class", "Species", "Clusters"))
output_file <- paste0(sample_name, "_consensus_tax_spc_ITS_treemap_clusters.png")
write_treemap(df, output_file, "Species")
