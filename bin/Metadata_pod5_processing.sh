...
select_and_validate_metadata_rows() {
	...
	awk -v run="$run_name" -v metadata_path="$metadata_path" -v row_info="$row_info_out" -v replicate_roster_rows="$replicate_roster_rows_out" -v samples_compat_rows="$samples_compat_rows_out" '
		...
		}
		{
			...
			if (sample_id ~ /\|/) add_problem("Sample_ID must not contain |")
			if (sample_id ~ /\//) add_problem("Sample_ID must not contain /")
			sample_id_lower = tolower(sample_id)
			if (sample_id_lower == "no_adapter" || sample_id_lower ~ /^no_adapter_[0-9]+$/) add_problem("Sample_ID must not use reserved no_adapter identity")
			if (pipeline_id ~ /[[:space:]]/) add_problem("Pipeline_ID must not contain whitespace")
			if (pipeline_id ~ /\|/) add_problem("Pipeline_ID must not contain |")
			if (pipeline_id ~ /\//) add_problem("Pipeline_ID must not contain /")
			pipeline_id_lower = tolower(pipeline_id)
			if (pipeline_id_lower == "no_adapter" || pipeline_id_lower ~ /^no_adapter_[0-9]+$/) add_problem("Pipeline_ID must not use reserved no_adapter identity")
