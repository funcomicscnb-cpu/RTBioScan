...
def test_run_index_update_replaces_existing(tmp_path: Path) -> None:
    ...
    old = {"run_id": "runA", "last_updated_utc": "2026-03-06T00:00:00Z", "rounds_count": 1}
    other = {"run_id": "runB", "last_updated_utc": "2026-03-06T00:00:00Z", "rounds_count": 2}
    index.write_text(json.dumps(old) + "\n" + json.dumps(other) + "\n", encoding="utf-8")

    new = {"run_id": "runA", "last_updated_utc": "2026-03-06T00:10:00Z", "rounds_count": 3}
    ...
    updated = [l for l in lines if l["run_id"] == "runA"][0]
    assert updated["rounds_count"] == 3
